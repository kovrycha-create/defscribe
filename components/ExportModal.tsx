...
import { type TranscriptEntry, type SpeakerProfile, type SpeakerId, type SpeechAnalytics, type DiarizationSettings, type ActionItem, type Snippet } from '../types';
...
const ExportModal: React.FC<ExportModalProps> = ({ 
    ...
    const generateFormattedTranscript = useCallback((forPdf: boolean = false): string => {
        ...
        if (includeTopics && topics.length > 0) {
            output += '--- KEY TOPICS ---' + nl + topics.join(', ') + nl + nl;
        }
        ...
        output += entriesText;
        return output;
    }, [includeStats, includeSpeakers, includeSummary, includeActionItems, includeSnippets, includeTopics, speechAnalytics, transcriptEntries, speakerProfiles, confidence, summary, actionItems, snippets, topics]);
