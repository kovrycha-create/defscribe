...
const NavButton: React.FC<{ icon: string; label: string; isActive: boolean; onClick: () => void; count?: number; }> = ({ icon, label, isActive, onClick, count }) => (
  <button onClick={onClick} className={`relative flex-1 flex flex-col items-center justify-center p-2 transition-all duration-200 ${isActive ? 'text-[var(--color-primary)]' : 'text-slate-400 hover:text-white'}`}>
    <i className={`fas ${icon} text-xl mb-1`}></i>
    <span className="text-xs font-semibold">{label}</span>
    {isActive && <div className="w-1.5 h-1.5 bg-[var(--color-primary)] rounded-full mt-1 shadow-[0_0_4px_var(--color-primary)]"></div>}
    {count && count > 0 ? (
        <span className="absolute top-1 right-1/2 translate-x-4 text-xs bg-[var(--color-secondary)] text-white rounded-full h-5 w-5 flex items-center justify-center font-bold">{count > 9 ? '9+' : count}</span>
    ) : null}
  </button>
);
...
const BottomNav: React.FC<BottomNavProps> = ({ activeTab, onTabChange, actionItemsCount, snippetsCount }) => {
  return (
    <nav className="flex-shrink-0 w-full bg-[rgba(10,18,24,0.8)] border-t border-[rgba(var(--color-primary-rgb),0.2)] flex items-center justify-around backdrop-blur-md z-30">
